class User < ActiveRecord::Base
  ...
  attr_accessible :login, :email, :password, :password_confirmation, :remember_me, :city_id, :sex, :profession, :aboutme, :birthday, :photo
  ...
  has_many :relationships, :foreign_key => "follower_id",
	...
	def self.commonsearch(search)  
		if search  
			where('login LIKE ? OR email LIKE ? ',"%#{search}%","%#{search}%").limit(self.paginationCount)
			#TODO sql injection possible???
		else
